...
class MarketplaceController extends GetxController {
  ...
  List<FarmListing> get filteredListings {
    return _allListings.where((listing) {
      ...
      final matchesSearch = query.isEmpty ||
          ...
          listing.products.any((p) => p.productId.toLowerCase().contains(query));
  ...
  bool get isLoading => _allListings.isEmpty && searchQuery.isEmpty; // Упрощенное состояние загрузки
  ...
  void updateSelectedDate(DateTime? date) {
    if (selectedDate.value == date) {
      selectedDate.value = null; // Toggles off if same date clicked? Or maybe user just wants a "Tümü" button.
    } else {
